...
<?php
 if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(empty($username) or empty($email) or empty($password) or !filter_var($email, 
    ...
    $query=mysqli_query($conn,"SELECT  username FROM register_test WHERE 
    username='$username';");
    $query1=mysqli_query($conn,"SELECT email FROM register_test WHERE 
    email='$email';");
    ...
    $query = mysqli_query($conn, "INSERT INTO register_test (username, email, password)
